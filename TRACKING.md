...
**Frontend Phase 2, onboarding verification + CSV bulk import (self-tested live 2026-09-15):** `frontend/PLAN.md` Phase 2 item 4, done **genuinely offline** rather than mocked — ARCHITECTURE.md §9 already requires onboarding to work fully offline, and GSTIN/PAN/IFSC format+checksum are real, complete, publicly documented algorithms, so there was nothing to fall back *from*. `lib/verification.ts`: GSTIN mod-36 check-digit algorithm verified against a real registered GSTIN found via web search (`33AAACC1206D1ZN`, Central Warehousing Corporation) before shipping it, after several half-remembered test vectors failed to match — worth flagging: don't trust memorized checksum test vectors, verify against a cited source; PAN format + holder-type decode; IFSC format + a curated ~35-bank prefix lookup (unrecognized-but-well-formed codes are shown as valid without a bank name, not flagged invalid); PIN code → coarse first-digit postal zone, explicitly labeled "(approximate, from PIN prefix)" since it's not a full locality database. Wired into `SellerProfilePage` (GSTIN/PAN/Udyam/pincode/IFSC, with "Use as primary state" and "Autofill bank name" one-click actions) and `MarketplaceAccountFormDialog` (warehouse pincode). Verified live end-to-end: typed a checksum-broken GSTIN → correctly rejected; corrected it → correctly decoded Maharashtra + Firm/LLP entity; clicked both autofill actions; saved the form; confirmed via `GET /api/sellers/me` that gstin/pan/primary_state/bank_ifsc/bank_name all persisted for real.

CSV bulk import (`ProductBulkImportDialog`, triggered from `ProductsPage`): parses client-side (required fields, numeric checks, `gst_rate` enum) and calls the real `POST /api/products` once per valid row — no bulk endpoint exists, so this is the same client-orchestration pattern as the multi-file settlement upload. Verified live with a 4-row CSV: 2 rows created for real, 1 row correctly skipped by local validation (missing SKU) before ever hitting the network, 1 row sent to the real API and correctly surfaced the backend's actual `"A product with this internal_sku already exists."` error inline — not a simulated error. Products list re-rendered the 2 new rows immediately (existing `useCreateProduct` cache invalidation, no new plumbing needed).
